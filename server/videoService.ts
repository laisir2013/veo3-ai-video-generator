import { getNextApiKey, API_ENDPOINTS, STORY_MODE_PRESETS, type VideoModel, type StoryMode } from "./videoConfig";
...
請分析這個故事並生成場景數據。記住旁白文字必須使用${langConfig.outputLanguage}。`;
  ...
  const fallbackModel = LLM_FALLBACK_CONFIG[llmModel as keyof typeof LLM_FALLBACK_CONFIG];
  if (fallbackModel) {
...
export async function generateSpeech(
  ...
  try {
    ...
    try {
      ...
      // 備用方案：使用 VectorEngine TTS
      const vectorVoice = language === "english" ? "nova" : "alloy";
      const audioUrl = await generateSpeechWithVectorEngine(text, vectorVoice, "tts-1-hd");
