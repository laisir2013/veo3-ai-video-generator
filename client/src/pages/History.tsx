...
export default function History() {
  ...
  // 獲取歷史任務列表
  const { data: historyData, isLoading, refetch } = trpc.video.getHistory.useQuery(
    { limit: 50, offset: 0 }
  );
  ...
  const history = historyData?.length ? historyData.map((task: any) => ({
    id: task.id,
    title: task.title || `任務 #${task.id}`,
    status: task.status,
